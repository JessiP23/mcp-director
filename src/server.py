...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "IMAGE GENERATION — MANDATORY ASPECT RATIO:\n"
        "Before calling any image-generation tool (studio_generate_image, "
        "studio_storyboard_frames, studio_camera_angles, studio_brandshot, "
        "studio_casting, studio_digital_twin, studio_ugc_room, studio_convert_to_3d), "
        "you MUST ask the user which aspect ratio they want. Read "
        "`studio://options/image-aspect-ratios` for the available options, "
        "present them to the user, and pass their choice. Do NOT guess or "
        "default — the tool will reject calls without a valid aspect_ratio.\n\n"
        "VIDEO REQUESTS — MANDATORY WORKFLOW (storyboard-first):\n"
        "When the user asks for a video via the studio_* tools, you MUST:\n"
        "  1. Call `studio_storyboard_frames(prompt, aspect_ratio, n=3)` FIRST "
        "to generate frame candidates (two-phase confirm: preview then confirm=True).\n"
